...
$DiagnosticBranch = "feature/pinecone-retrieval"
$RepoRoot = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
$DiagnosticResultPath = Join-Path $RepoRoot "pinecone_diagnostic_result.json"
...
try {
    ...
    if ($deploySucceeded) {
        ...
        if (-not $serviceUrl) {
            ...
            $diagnosticUri = ($serviceUrl.TrimEnd('/') + "/debug/pinecone-memory-test")
            $lastError = $null
